...
class CardEditor:
    ...
    def apply_content_aware_fill(self):
        ...
        # Create a dialog to get color influence parameters
        fill_dialog = tk.Toplevel(self.root)
        fill_dialog.title("Content-Aware Fill")
        fill_dialog.geometry("400x230")
        fill_dialog.transient(self.root)
        fill_dialog.grab_set()
        
        # Variables
        color_var = tk.StringVar(value="#000000")
        influence_var = tk.DoubleVar(value=0.0)  # 0 = pure inpainting, 1 = pure color
        eyedropper_active = [False]  # Using list for mutable reference
        ...
        # Function to activate eyedropper
        def activate_eyedropper():
            eyedropper_active[0] = True
            fill_dialog.grab_release()  # Allow interaction with main window
            self.canvas.config(cursor="crosshair")
            self.status_label.config(text="Click on image to sample color")
            
            # Store original bindings
            original_click = self.canvas.bind("<Button-1>")
            
            # Create new binding for color picking
            def pick_color_from_image(event):
                if not eyedropper_active[0]:
                    return
                ...
                if 0 <= image_x < self.img_width and 0 <= image_y < self.img_height:
                    ...
                    try:
                        ...
                        self.canvas.config(cursor="")
                        eyedropper_active[0] = False
                        fill_dialog.grab_set()  # Restore dialog as modal
                        self.status_label.config(text="Color sampled")
                        
                        # Restore original binding
                        self.canvas.bind("<Button-1>", original_click)
                    except Exception as e:
                        print(f"Error sampling color: {e}")
            
            # Set temporary binding
            self.canvas.bind("<Button-1>", pick_color_from_image)
        
        # Function to apply the fill
        def apply_fill():
            color_value = color_var.get()
            influence = influence_var.get()
            fill_dialog.destroy()
            ...
            print(f"Color influence: {color_value} at {influence*100}% strength")
            ...
            try:
                ...
                # If influence > 0, blend with color
                if influence > 0:
                    # Create a color overlay
                    color_overlay = np.zeros_like(result)
                    color_overlay[:] = (b, g, r)  # BGR format for OpenCV
                    
                    # Create alpha mask from the mask (0-1 range)
                    alpha = mask.astype(float) / 255.0
                    
                    # Extend alpha to 3 channels
                    alpha_3channel = np.stack([alpha, alpha, alpha], axis=2)
                    
                    # Blend inpainted result with color based on influence
                    blend_mask = alpha_3channel * influence
                    result = (1 - blend_mask) * result + blend_mask * color_overlay
                    
                    print(f"Blended with color at {influence*100}% influence")
        ...
        def cancel_fill():
            fill_dialog.destroy()
            if eyedropper_active[0]:
                self.canvas.config(cursor="")
                eyedropper_active[0] = False
        
        # Create UI
        frame = ttk.Frame(fill_dialog, padding=10)
        frame.pack(fill='both', expand=True)
        
        # Color influence
        ttk.Label(frame, text="Influence Color:").grid(row=0, column=0, sticky='w', pady=10)
        color_frame = ttk.Frame(frame)
        color_frame.grid(row=0, column=1, sticky='w', pady=10)
        ...
        eyedropper_img = None
        try:
            # Create a simple eyedropper icon
            icon_size = 16
            icon = Image.new('RGBA', (icon_size, icon_size), (0, 0, 0, 0))
            draw = ImageDraw.Draw(icon)
            
            # Draw a simple eyedropper icon
            # Dropper tip
            draw.line([(3, 12), (8, 7)], fill="black", width=2)
            # Dropper body
            draw.rectangle([7, 3, 12, 8], outline="black", fill="white")
            # Dropper top
            draw.rectangle([8, 2, 11, 4], outline="black", fill="white")
            
            eyedropper_img = ImageTk.PhotoImage(icon)
            eyedropper_btn = ttk.Button(color_frame, image=eyedropper_img, command=activate_eyedropper)
            eyedropper_btn.image = eyedropper_img  # Keep a reference
            eyedropper_btn.pack(side=tk.LEFT)
        except:
            # Fallback to text if creating image fails
            eyedropper_btn = ttk.Button(color_frame, text="🔍", command=activate_eyedropper)
            eyedropper_btn.pack(side=tk.LEFT)
        
        # Add information tooltip
        ttk.Label(color_frame, text="Pick from image", foreground="blue").pack(side=tk.LEFT, padx=5)
        
        # Influence strength
        ttk.Label(frame, text="Color Influence:").grid(row=1, column=0, sticky='w', pady=10)
        influence_frame = ttk.Frame(frame)
        influence_frame.grid(row=1, column=1, sticky='we', pady=10)
        influence_scale = ttk.Scale(influence_frame, from_=0, to=1.0, variable=influence_var, orient='horizontal')
        influence_scale.pack(side=tk.LEFT, fill='x', expand=True)
        influence_label = ttk.Label(influence_frame, text="0%")
        influence_label.pack(side=tk.LEFT, padx=5)
        
        def update_influence_label(*args):
            influence_label.config(text=f"{int(influence_var.get() * 100)}%")
        
        influence_var.trace_add("write", update_influence_label)
        
        # Info text
        info_text = "0% = Pure inpainting (based on surrounding pixels)\n100% = Pure color fill"
        ttk.Label(frame, text=info_text, justify=tk.LEFT).grid(row=2, column=0, columnspan=2, sticky='w', pady=10)
        
        # Buttons
        button_frame = ttk.Frame(frame)
        button_frame.grid(row=3, column=0, columnspan=2, pady=10)
        ttk.Button(button_frame, text="Apply", command=apply_fill).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Cancel", command=cancel_fill).pack(side=tk.LEFT, padx=5)
        
        # Make sure dialog is properly cleaned up on window close
        fill_dialog.protocol("WM_DELETE_WINDOW", cancel_fill)
